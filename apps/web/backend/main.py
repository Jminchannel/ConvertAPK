...
CDN_LOCALIZE_SCRIPT_PATH = Path(__file__).resolve().parents[3] / "workers" / "apk-worker" / "scripts" / "offlineize_html_assets.mjs"
...
def _resolve_quick_generate_icon_path() -> Path | None:
    ...
    try:
        root_from_file = Path(__file__).resolve().parents[3]
